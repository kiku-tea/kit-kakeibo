...
|粒度|役割|抽象度|関心|store・API参照|
|--|:-|:-:|:-:|:-:|
|atoms|これ以上分解できない最小単位|大|レイアウト|×|
|molecules|複数のコンポーネントからの利用を想定したもの|大|レイアウト|×|
|organisms|各ページに特化したセクショニング部品|ページ依存|コンテンツ|◯|
|templates|ページの枠組みを構成|プロジェクト依存|レイアウト|×|
|pages|ページのコンテンツの状態を管理|プロジェクト依存|コンテンツ|◯|
